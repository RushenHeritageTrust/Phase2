...
public class Blowfish {
	...
	private static class BlowfishCBC extends BlowfishECB {
		...
		public void decrypt(byte[] inbuffer, byte[] outbuffer) {
			final int nLen = inbuffer.length;
			long lTemp;
			for (int nI = 0; nI < nLen; nI += 8) {
	...
	private static class BlowfishECB {
		...
		/** The Constant PBOX_ENTRIES. */
		final static int PBOX_ENTRIES = 18;

		/** The Constant pbox_init. */
		...
		/** The Constant SBOX_ENTRIES. */
		final static int SBOX_ENTRIES = 256;

		/** The Constant sbox_init_1. */
		...
		protected long encryptBlock(long lPlainBlock) {
			...
			final int[] sbox1 = m_sbox1;
			final int[] sbox2 = m_sbox2;
			final int[] sbox3 = m_sbox3;
			final int[] sbox4 = m_sbox4;

			final int[] pbox = m_pbox;
	...
	private static long intArrayToLong(int[] buffer, int nStartIndex) {
		return (long) buffer[nStartIndex] << 32 | buffer[nStartIndex + 1]
				& 0x0ffffffffL;
	}
	...
	/** The m_bfish. */
	private final BlowfishCBC m_bfish;
	...
	public Blowfish(String password) {
		...
		try {
			...
			// Logger.getLogger(Blowfish.class.getName()).log(Level.SEVERE,
			// null, e);
		}
	...
	public String decryptString(String sCipherText) {
		...
		// get the CBC IV
		final byte[] cbciv = new byte[BlowfishECB.BLOCKSIZE];
		...
		nNumOfBytes = binHexToBytes(sCipherText, buf,
				BlowfishECB.BLOCKSIZE * 2, 0, nLen);
	...
	private String encStr(String sPlainText, long lNewCBCIV) {
		...
		final byte bPadVal = (byte) (buf.length - (nStrLen << 1));
		...
		// return the binhex string
		final byte[] newCBCIV = new byte[BlowfishECB.BLOCKSIZE];
